...
async def single_file_cmds(c, m):
    ...
    try:
        ...
        if cmd == "compress":
            ...
            out = await unzip_file(path) # Returns directory
...
# --- 4. LISTENER (Saves first file for merge) ---
...
# --- 5. MERGE COMMANDS (Needs 2 files) ---
...
async def merge_cmds(c, m):
    if m.from_user.id not in MERGE_DICT:
        return await m.reply_text("❌ Pehle first file (Photo/Audio/Sub/Video) bhejo, fir Video pe reply karke command do.")
    if not m.reply_to_message: return await m.reply_text("❌ Video par reply karo!")
    
    cmd = m.command[0]
    file1_msg = MERGE_DICT[m.from_user.id] # Cached File
    file2_msg = m.reply_to_message       # Reply Message (Video)
    
    msg = await m.reply_text("⬇️ **Downloading Files...**")
    try:
        path1 = await file1_msg.download(progress=progress_bar, progress_args=(msg, "DL File 1"))
        path2 = await file2_msg.download(progress=progress_bar, progress_args=(msg, "DL Video"))
        out = None
        
        if cmd == "watermark":
            await msg.edit_text("🖼 **Adding Watermark...**")
            out = await add_watermark(path2, path1) # Path1 is Image, Path2 is Video
        elif cmd == "add_sub":
            await msg.edit_text("📝 **Adding Subtitles...**")
            out = await add_subtitle(path2, path1) # Path1 is SRT
        elif cmd == "merge_audio":
            await msg.edit_text("🎵 **Merging Audio...**")
            out = await merge_audio(path2, path1) # Path1 is Audio
        elif cmd == "merge_videos":
            await msg.edit_text("🎞 **Joining Videos...**")
            out = await join_videos(path2, path1) # Join Reply(2) + Cached(1)
